...
struct ElemNode : public  Node< ElemNode< Crate> >
{ 
...
template <typename ParentForge>
    ...
    template < typename Cnstr>
        auto        Setup( ElemNode *node, Cnstr *cnstr)
        {
            m_Elem = m_Entry;       ; 
            return this;
        } 
...
template < typename TNode>
struct NodePtr : public  Node< NodePtr< TNode> >
{
...
public:
    NodePtr( TNode *node)
        : m_PtrNode( node->GetNode()) 
    {}
        
template <typename ParentForge>
    bool DoMatch( ParentForge *ctxt) const
    { 
        return m_PtrNode->DoMatch( ctxt);  
    } 
    ...
    template < typename Cnstr>
        auto        Setup( NodePtr *node, Cnstr *cnstr)
        {
            m_Elem = cnstr->FetchSynTree( node->m_PtrNode);       ; 
            return this;
        } 
    };
};
