...
#include    "segue/epigraph/sg_shard.h"
...
struct  StrInStream
{
    ...
    std::vector< uint32_t>      m_NLOffsets;

	StrInStream( void)
		: m_Cursor( 0), m_NLOffsets( 1, 0)
	{}
    ...
    void                ExamineNL(  void)
    {
        m_NLOffsets.resize( 0);
        for ( uint32_t ind = 0; ind < m_CharVec.size(); ++ind )
            if ( m_CharVec[ ind] == '\n')
                m_NLOffsets.push_back( ind);
    }

    Cv_Couple< uint32_t>        LineColumn( void) const
    {
        uint32_t        line = uint32_t( std::lower_bound( m_NLOffsets.begin(), m_NLOffsets.end(), m_Cursor) -m_NLOffsets.begin());
        uint32_t        col = m_Cursor -m_NLOffsets[ line -1];
        return std::make_tuple( line +1, col);
    }
    bool                        SkipToLine( uint32_t line)
    {
        if ( --line < m_NLOffsets.size())
        {
            m_Cursor = m_NLOffsets[ line] +1;
            return m_Cursor < m_CharVec.size();
        }
        return false;
    }
};
...
template < typename TimbreShard>
    ...
    bool        SkipToLine( uint32_t line)
    {
        bool    retValue = GetParser()->SkipToLine( line);
        m_Marker = GetParser()->Curr();
        return retValue;
    }
};
...
public:
    ...
    Cv_Couple< uint32_t>        LineColumn( void) const { return m_InStream->LineColumn(); }
    bool                        SkipToLine( uint32_t line) const { return m_InStream->SkipToLine( line); }
